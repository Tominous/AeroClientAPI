...
public class ClientBanCommand implements CommandExecutor {
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    if(!SpigotConfig.bungee && !MinecraftServer.getServer().getOnlineMode()) {
      sender.sendMessage(ChatColor.BLUE + "[API]: " + ChatColor.RED + "You may not ban that player while server is not using online mode");
    ...
    target.kickPlayer(ChatColor.BLUE + "[API]: " + ChatColor.RED + "You have been Aero Client banned.");
    target.setBanned(true);
    Bukkit.getServer().broadcastMessage(ChatColor.STRIKETHROUGH + "-------------------------------------------");
    Bukkit.getServer().broadcastMessage(ChatColor.BLUE + "[API]: " + target.getName() + " has been Aero Client Banned");
